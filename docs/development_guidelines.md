...
Фильтры имеют встроенную функциональность поиска через поля `.filter-search__input`. Каждое поле поиска работает независимо для своего блока фильтров.
...
1. **Управление позициями активных элементов**: Пункты с классом `.filter__item_active` управляются автоматически:
   ...
   - **При выборе элемента**: элемент перемещается в начало списка **с задержкой 600мс** (плавный эффект)
...
- **При добавлении active**: элемент перемещается наверх через `moveActiveItemsToTop()` с задержкой **600мс**
...
2. Пользователь выбирает "Яндекс" → **через 600мс** элемент плавно перемещается наверх: [Яндекс, Все, Рамблер, Instagram, Avito...]
3. Пользователь выбирает "Avito" → **через 600мс** элемент плавно перемещается наверх: [Avito, Яндекс, Все, Рамблер, Instagram...]
...
- **Обработчик поиска**: `input` event на `.filter-search__input`
- **Обработчик кликов**: `click` event на `.filter__item`:
  ...
  - Если стал активным → перемещает наверх с задержкой **600мс** (для визуального эффекта)
